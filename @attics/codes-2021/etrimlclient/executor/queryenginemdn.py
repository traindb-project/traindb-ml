...
from etrimlclient.ml.integral import (approx_avg, approx_count,
                                     approx_integrate, approx_sum,
                                     prepare_reg_density_data, prepare_var)
from etrimlclient.ml.mdn import KdeMdn, RegMdnGroupBy
from etrimlclient.ml.modeltrainer import KdeModelTrainer
from etrimlclient.socket import app_client
from etrimlclient.tools.running_parameters import shrink_runtime_config
...
# from etrimlclient.tools.dftools import get_group_count_from_summary_file
...
class GenericQueryEngine:
    def __init__(self):
        #print(">>> executor > queryenginemdn.py > GenericQueryEngine : __init__()")

        self.mdl_name = None

    def serialize2warehouse(self, warehouse, runtime_config):
        #print(">>> executor > queryenginemdn.py > GenericQueryEngine : serialize2warehouse()")

        with open(warehouse + '/' + self.mdl_name + runtime_config["model_suffix"], 'wb') as f:
            dill.dump(self, f)

    def init_pickle_file_name(self, runtime_config):
        #print(">>> executor > queryenginemdn.py > GenericQueryEngine : init_pickle_file_name()")
    ...
    def fit(self, mdl_name: str, origin_table_name: str, data: dict, total_points: dict, usecols: dict, runtime_config: dict):
        #print(">>> executor > queryenginemdn.py > GenericQueryEngine : fit()")

        pass

    def predicts(self, func: str, x_lb: float, x_ub: float, x_categorical_conditions, runtime_config, groups: list = None, filter_etriml=None):
        #print(">>> executor > queryenginemdn.py > GenericQueryEngine : predicts()")
...
class MdnQueryEngineNoRange(GenericQueryEngine):
    def __init__(self,  config):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngineNoRange : __init__()")
    ...
    def fit(self, mdl_name: str, origin_table_name: str, data: dict, total_points: dict, usecols: dict, runtime_config: dict):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngineNoRange : fit()")
    ...
    def predicts(self, func: str, x_lb: float, x_ub: float, x_categorical_conditions, runtime_config, groups: list = None, filter_etriml=None):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngineNoRange : predicts()")
...
class MdnQueryEngineRangeNoCategorical(GenericQueryEngine):
    def __init__(self,  config):
        #print(">>> executor > queryenginemdn.py > MdnQuMdnQueryEngineRangeNoCategoricaleryEngineNoRange : __init__()")
    ...
    def fit(self, mdl_name: str, origin_table_name: str, gbs_data, xs_data, ys_data, total_points: dict, usecols: dict, runtime_config: dict):
        #print(">>> executor > queryenginemdn.py > MdnQuMdnQueryEngineRangeNoCategoricaleryEngineNoRange : fit()")
    ...
    def predicts(self, func: str, x_lb: float, x_ub: float, x_categorical_conditions, runtime_config, groups: list = None, filter_etriml=None):
        #print(">>> executor > queryenginemdn.py > MdnQuMdnQueryEngineRangeNoCategoricaleryEngineNoRange : predicts()")
        ...
        if func.lower() in ["count", "sum", "avg"]:
            if n_jobs == 1:
                ...
                # new_ft={}
                # with open("/home/quincy/Documents/workspace/ETRIMLClient/etrimlwarehouse/large_group_counts.csv", "r") as f:
                #     for line in f:
                #         splits = line.split(",")
                #         key="_".join(splits[:-1])
                #         value = int(splits[-1])
...
class MdnQueryEngineNoRangeCategorical(GenericQueryEngine):
    ...
    def predicts(self, func: str, x_lb: float, x_ub: float, x_categorical_conditions, runtime_config, groups: list = None, filter_etriml=None) -> pd.DataFrame:
        #print(">>> executor > queryenginemdn.py > MdnQuMdnQueryEngineRangeNoCategoricaleryEngineNoRange : predicts()")
        ...
        if not x_categorical_conditions[2]:
            ...
            predictions = self.models[key].predicts(
                func, x_lb=x_lb, x_ub=x_ub, x_categorical_conditions=x_categorical_conditions, runtime_config=runtime_config, filter_etriml=filter_etriml)
            return predictions
        else:
...
class MdnQueryEngineNoRangeCategoricalOneModel(GenericQueryEngine):
    def __init__(self, config):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngineNoRangeCategoricalOneModel : __init__()")
    ...
    def fit(self, mdl_name: str, origin_table_name: str, gbs, xs, ys, total_points: dict, usecols: dict, runtime_config: dict):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngineNoRangeCategoricalOneModel : fit()")
    ...
    def predicts(self, func: str, x_lb: float, x_ub: float, x_categorical_conditions, runtime_config, groups: list = None, filter_etriml=None):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngineNoRangeCategoricalOneModel : predicts()")
...
class MdnQueryEngine(GenericQueryEngine, mlflow.pyfunc.PythonModel):
    t_before_multiple_processing = None

    
    def load_context(self, context):
        with open(context.artifacts["etriml_model"], "rb") as f:
            self.etriml_model = dill.load(f)
        #print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!", type(self.etriml_model))

    def predict(self, context, params): #func: str, x_lb: float, x_ub: float,  where_conditions, runtime_config, groups: list, filter_etriml):
        predictions = self.etriml_model.predicts_x(params,)

        return predictions    


    def __init__(self, kdeModelWrapper, config):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngine : __init__()")
    ...
    def approx_avg(self, x_min, x_max, groupby_value, runtime_config):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngine : approx_avg()")
    ...
    def approx_sum(self, x_min, x_max, groupby_value, runtime_config):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngine : approx_sum()")
    ...
    def approx_count(self, x_min, x_max, groupby_value, runtime_config):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngine : approx_count()")
    ...
    def approx_var(self, runtime_config):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngine : approx_var()")
    ...
    def predict(self, func, x_lb=None, x_ub=None, groupby_value=None, runtime_config=None):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngine : predict()")
    ...
    def predicts(self, func: str, x_lb: float = None, x_ub: float = None,  x_categorical_conditions=None, runtime_config=None, groups: list = None, filter_etriml=None, time2exclude_from_multiprocessing=None):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngine : predicts()")
        ...
        if self.config.get_config()["accept_filter"]:
            ...
            results = {key: results[key] for key in results if float(
                key) >= filter_etriml[0] and float(key) <= filter_etriml[1]}
        ...
        if func.lower() in ["count", "sum", "avg"]:
            if n_jobs == 1:
                ...
                # exit()
                # print("groups", groups)
                pre_density, pre_reg, step = prepare_reg_density_data(self.kde, x_lb, x_ub, groups=groups, reg=self.reg, runtime_config=runtime_config)
                # print("pre_density, pre_reg", pre_density,)
                # print(pre_reg)
                # exit()
                ...
                if runtime_config["slaves"].is_empty():
                    ...
                    for sub_group in group_chunks:
                        ...
                        i = pool.apply_async(
                            self.predicts, (func, x_lb, x_ub, x_categorical_conditions, runtime_config_process, sub_group, filter_etriml, time2exclude_from_multiprocessing))
                        instances.append(i)
                    ...
                    for sub_group, host in zip(group_chunks, hosts):
                        ...
                        query = dict(func=func, x_lb=x_lb, x_ub=x_ub, x_categorical_conditions=x_categorical_conditions, runtime_config=runtime_config_process,
                                     sub_group=sub_group, filter_etriml=filter_etriml, mdl_name=self.mdl_name+runtime_config["model_suffix"])
                        i = pool.apply_async(
    ...
    def predicts_x(self, params):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngine : predicts_x()")
        #print("type(params) : ", type(params))
        #print("params : \n", params)

    
        df = params.where(pd.notnull(params), None)
        func = df.iloc[0][0]
        x_lb = df.iloc[1][0]
        x_ub = df.iloc[2][0]
        x_categorical_conditions=df.iloc[3][0]
        runtime_config=df.iloc[4][0]
        groups = df.iloc[5][0]
        filter_etriml=df.iloc[6][0]
        time2exclude_from_multiprocessing=None

        print()
        print()
        print("======================================================================")
        print("PARAMESTERS : ", type(params))
        print("----------------------------------------------------------------------")
        print("func\t: ", func)
        print("x_lb\t: ", x_lb)
        print("x_ub\t: ", x_ub)
        print("where\t: ", x_categorical_conditions)
    ...
    def serialize2warehouse(self, warehouse, runtime_config):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngine : serialize2warehouse()")

        with open(warehouse + '/' + self.mdl_name + runtime_config["model_suffix"], 'wb') as f:
            dill.dump(self, f)

    def init_pickle_file_name(self, runtime_config):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngine : init_pickle_file_name()")

        return self.mdl_name+runtime_config["model_suffix"]


def query_partial_group(mdnQueryEngine, group, func, x_lb, x_ub):
    #print(">>> executor > queryenginemdn.py > None : query_partial_group()")
...
class MdnQueryEngineGoGs():
    ...
    def __init__(self, config):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngineGoGs : __init__()")
    ...
    def fit(self, df: pd.DataFrame, groupby_attribute: str, n_total_point: dict,
        ...
        #print(">>> executor > queryenginemdn.py > MdnQueryEngineGoGs : fit()")
    ...
    def predicts(self, func, x_lb, x_ub, x_categorical_conditions, runtime_config, groups: list = None, filter_etriml=None):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngineGoGs : predicts()")
        ...
        for index, sub_group in enumerate(self.group_keys_chunk):
            ...
            i = pool.apply_async(
                engine.predicts, (func, x_lb, x_ub, x_categorical_conditions, runtime_config, sub_group, filter_etriml))
            instances.append(i)
    ...
    def init_pickle_file_name(self, runtime_config):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngineGoGs : init_pickle_file_name()")

        # self.pickle_file_name = self.pickle_file_name
        return self.pickle_file_name + runtime_config["model_suffix"]

    def serialize2warehouse(self, warehouse, runtime_config):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngineGoGs : serialize2warehouse()")
...
class MdnQueryEngineXCategorical(GenericQueryEngine):
    ...
    def __init__(self, config):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngineXCategorical : __init__()")
    ...
    def fit(self, mdl_name: str, origin_table_name: str, data: dict, total_points: dict, usecols: dict, runtime_config: dict):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngineXCategorical : fit()")
    ...
    def predicts(self, func, x_lb, x_ub, x_categorical_conditions, runtime_config, groups: list = None, filter_etriml=None,):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngineXCategorical : predicts()")
                ...
                for value in distinct_values:
                    if meet_condition(value, x_categorical_conditions[2][col]):
                        ...
                        pred = self.models[key].predicts(func, x_lb=x_lb, x_ub=x_ub, x_categorical_conditions=x_categorical_conditions,
                                                         runtime_config=runtime_config, filter_etriml=filter_etriml)
    ...
    def serialize2warehouse(self, warehouse, runtime_config):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngineXCategorical : serialize2warehouse()")

        with open(warehouse + '/' + self.mdl_name + runtime_config["model_suffix"], 'wb') as f:
            dill.dump(self, f)

    def init_pickle_file_name(self, runtime_config):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngineXCategorical : init_pickle_file_name()")

        return self.mdl_name+runtime_config["model_suffix"]


def meet_condition(value: str, condition):
    #print(">>> executor > queryenginemdn.py > None : meet_condition()")
...
class MdnQueryEngineXCategoricalOneModel(GenericQueryEngine):
    def __init__(self, config):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngineXCategoricalOneModel : __init__()")
    ...
    def serialize2warehouse(self, warehouse, runtime_config):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngineXCategoricalOneModel : serialize2warehouse()")

        with open(warehouse + '/' + self.mdl_name + runtime_config["model_suffix"], 'wb') as f:
            dill.dump(self, f)

    def init_pickle_file_name(self, runtime_config):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngineXCategoricalOneModel : init_pickle_file_name()")

        return self.mdl_name+runtime_config["model_suffix"]

    def fit(self, mdl_name: str, origin_table_name: str, gbs, xs, ys, total_points: dict, usecols: dict, runtime_config: dict):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngineXCategoricalOneModel : fit()")
    ...
    def predicts(self, func: str, x_lb: float, x_ub: float, x_categorical_conditions, runtime_config, groups: list = None, filter_etriml=None):
        #print(">>> executor > queryenginemdn.py > MdnQueryEngineXCategoricalOneModel : predicts()")
...
class QueryEngineFrequencyTable(GenericQueryEngine):
    def __init__(self,  config):
        #print(">>> executor > queryenginemdn.py > QueryEngineFrequencyTable : __init__()")
    ...
    def fit(self, mdl_name: str, origin_table_name: str, gbs_data,xs_data,ys_data, total_points: dict, usecols: dict, runtime_config: dict):
        #print(">>> executor > queryenginemdn.py > QueryEngineFrequencyTable : fit()")
    ...
    def predicts(self, func: str, x_lb: float, x_ub: float, x_categorical_conditions, runtime_config, groups: list = None, filter_etriml=None):
        #print(">>> executor > queryenginemdn.py > QueryEngineFrequencyTable : predicts()")
