...
from etrimlclient.cli.prompt import ETRIMLPrompt
from etrimlclient.executor.executor import SqlExecutor
from etrimlclient.ml import mdn
from etrimlclient.socket import app_client, app_server
...
def cmd():
    print("Welcome to ETRIML++")

    parser = argparse.ArgumentParser(
        description='Process the input for ETRIML++.')
...
def slave():
    ...
    if len(sys.argv) != 2:
        print("Usage:", " etrimlslave ", "<local IP>:<port>")
        print("Abort.")
        sys.exit(1)
