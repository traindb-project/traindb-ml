...
from etrimlclient.ml.density import ETRIMLDensity
from etrimlclient.ml.mdn import KdeMdn, RegMdnGroupBy  # RegMdn
from etrimlclient.ml.modelwraper import (GroupByModelWrapper, KdeModelWrapper,
                                        SimpleModelWrapper)
from etrimlclient.ml.regression import ETRIMLReg
from etrimlclient.tools.dftools import convert_df_to_yx
...
class SimpleModelTrainer:
    ...
    def __init__(self, mdl, tbl, xheader, yheader, n_total_point, n_sample_point, groupby_attribute=None,
        ...
        #print(">>> ml > modeltrainer.py > SimpleModelTrainer : __init__()")
    ...
    def fit(self, x, y, runtime_config):
        #print(">>> ml > modeltrainer.py > SimpleModelTrainer : fit()")

        # print(x,y)
        # if x_reg is None:
        #     reg = ETRIMLReg(config=self.config).fit(x_kde, y_kde)
        # else:
        reg = ETRIMLReg(config=self.config).fit(x, y, runtime_config)
        density = ETRIMLDensity(config=self.config).fit(x, None, runtime_config)
    ...
    def fit_from_df(self, df, runtime_config):
        #print(">>> ml > modeltrainer.py > SimpleModelTrainer : fit_from_df()")
...
class GroupByModelTrainer:
    def __init__(self, mdl, tbl, xheader, yheader, groupby_attribute, n_total_point, n_sample_point,
        ...
        #print(">>> ml > modeltrainer.py > GroupByModelTrainer : __init__()")
    ...
    def fit_from_df(self, df, runtime_config):
        #print(">>> ml > modeltrainer.py > GroupByModelTrainer : fit_from_df()")
...
class KdeModelTrainer:
    def __init__(self, mdl, tbl, xheader, yheader, groupby_attribute, groupby_values, n_total_point,
                 x_min_value=-np.inf, x_max_value=np.inf, config=None):
        #print(">>> ml > modeltrainer.py > KdeModelTrainer : __init__()")
    ...
    def fit_from_df(self, df, runtime_config, network_size=None,  b_shuffle_data=True):
        #print(">>> ml > modeltrainer.py > KdeModelTrainer : fit_from_df()")
        ...
        # density = ETRIMLDensity(config=self.config).fit(x)
