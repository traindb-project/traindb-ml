...
parser.add_argument('-s', '-sql', nargs='?', dest='sql', default='select model avg (add_to_cart_order real) from instacart_order_product_600k where 0.9 <= reordered <= 1.0', help='SQL query (DDL/DML)')
...
#sql_string = "create model instacart_order_product_10m (add_to_cart_order real, reordered real) from instacart_order_product_10m.csv method uniform size 1000"
...
sql_string = "select model sum (add_to_cart_order real) from instacart_order_product_10m where 0.9 <= reordered <= 1.0"
